...
研究管线会从时间切分生成独立的 `parameter_selection_sensitivity_ablation` 快照：仅含验证期末及以前的行情，且不继承源快照元数据。参数敏感性和因子消融不得接收完整快照。报告记录实验输入日期 hash、起止日期与最终测试起点，以便机器审计零重叠。

基线数据采用 `research-baselines/v1` 显式契约，包含沪深300、中证全指、简单动量三条逐日收益、来源和PIT声明。研究层不从股票池收益构造指数，也不对横截面收益乘系数伪造动量。契约缺失或未覆盖最终测试日期时，基线和超额门禁失败关闭。
