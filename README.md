...
仓库根目录包含生产容器配置。默认以 AKShare 的 30 只显式观察池启动，最终组合仍严格控制为 3～5 只；API 仅暴露在容器网络内，由 Nginx 同源代理 `/api`，SQLite 与研究制品保存在 Docker 持久卷中。
