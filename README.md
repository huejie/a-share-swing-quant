...
- 可审计通知发件箱，以及可选的邮件/Webhook 日终与风险告警。
...
Tushare 会校验请求日是 SSE 交易日，抓取最近 120 个交易日的日线、复权因子、行业、估值截面和指数代理；默认每次请求间隔 1.25 秒，完整加载约 5 分钟。没有 token 时也可使用 AKShare：它先读取当前全A实时/最近收盘快照，排除北交所、B股、ST/退市标记、无效价格和低成交额股票，以70%流动性分位和30%趋势代理稳定排序，最多保留60只；取得当前行业与上市日期、剔除上市不足120天的股票后，才逐只读取历史。该快照明确标记为非 PIT，历史 `as_of` 请求不会使用它。全A端点或动态候选行业元数据不可用时，适配器整体回退 `QUANT_AKSHARE_SYMBOLS` 与 `QUANT_AKSHARE_METADATA_PATH` 的30只审计池，并在 `universe_selection` 写明原因，不会静默改用 Demo。东方财富前复权日线失败时会切换到 AKShare 新浪前复权日线，并统一校正成交量为“股”；实际来源逐标的写入 `price_history` 审计。公开源只有在复权、覆盖率、行业分组和新鲜度等核心门禁全部通过时，才会生成明确标记为 `observation_only` 的决策观察；只有同时取得完整涨跌停与停牌约束时才生成/撮合模拟意图，AKShare 当前默认不撮合。它仍是“非 PIT、非生产”，不能通过生产研究门禁、不能替代授权数据包，也不会开启自动交易。token 不会写入数据库、前端或日志；不要写入 `.env.example` 或提交到仓库。
...
仓库根目录包含生产容器配置。默认以 AKShare 动态预选最多60只、30只显式池安全回退启动，最终组合仍严格控制为3～5只；API 仅暴露在容器网络内，由轻量 Node 静态网关同源代理 `/api`，SQLite 与研究制品保存在 Docker 持久卷中。
...
首次访问仪表盘会抓取 AKShare 数据，可能需要几十秒到数分钟。公开网页接口可能限流或临时不可用；这种情况下服务会明确降级/失败，不会静默回退 Demo。当前仅部署 HTTP，网关默认只开放公网读取，管理写操作要求个人管理密钥且在公网入口关闭；不要在 HTTP 页面输入密钥。面向远程管理前应配置域名、HTTPS 并完成访问控制评审。Linux 部署同时启用工作日日终与每日宿主机外置 SQLite 备份定时器。

通知默认不外发。启用前只在服务器 `.env` 配置 `QUANT_NOTIFICATION_WEBHOOK_URL`，或 `QUANT_NOTIFICATION_SMTP_*` 与 `QUANT_NOTIFICATION_EMAIL_FROM/TO`；再通过受管理密钥保护的设置接口选择 `webhook` 或 `email`。目标和凭据不会进入设置记录、通知载荷、决策日志或前端。投递失败会在通知发件箱中标记为 `failed`，不会使日终决策失败。
